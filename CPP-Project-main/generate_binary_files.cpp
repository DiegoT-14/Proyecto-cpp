...
int ReadAndWriteNumberOfMachines(fstream& text_file, fstream& binary_file) {
    char number_of_machines_char = ReadSingleByteNumber(text_file, "Number of machines");
    unsigned int number_of_machines = static_cast<unsigned int>(number_of_machines_char);
    binary_file.write(reinterpret_cast<const char*>(&number_of_machines), sizeof(number_of_machines));
...
void ProcessMeasurement(fstream& text_file, fstream& binary_file) {
    ...
    WriteStringToBinary(binary_file, patient_id, 1);  // Write only 1 byte for patient ID
    delete[] patient_id;
...
void ProcessReading(fstream& text_file, fstream& binary_file) {
    ...
    if (strcmp(reading_type, "T") == 0 || 
        ...
        // Pressure readings are stored as two doubles
        char* systolic_str = strtok(reading_value, "-");
        char* diastolic_str = strtok(nullptr, "-");
        
        double systolic = atof(systolic_str);
        double diastolic = atof(diastolic_str);
